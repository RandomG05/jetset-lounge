		...
		if(!empty($get_opt['produk_expired'])){
			if($get_opt['produk_expired'] != 'unlimited'){
				...
				if($today_mktime >= $produk_expired_alert){
					...
					if($sisa_hari >= 0){
						$r = array('success' => false, 'info' => 'Masa berlaku aplikasi SPOS.id anda akan berakhir<br/>pada tanggal: <b>'.$get_opt['produk_expired'].', <font color="red">'.$sisa_hari_text.' lagi</font></b><br/><br/>Silakan lakukan perpanjangan aplikasi</br>via website: <b>https://wepos.id</b><br/><br/>untuk pertanyaan seputar masa aktif hubungi<br/>CS: <b>0812-2254-9676</b></br>');
						die(json_encode($r));
					}
				}
			}
